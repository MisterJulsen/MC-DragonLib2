...
import de.mrjulsen.mcdragonlib.client.newgui.widgets.components.DLContextMenu.MenuBuilder;
...
public class DLRichTextLabel extends DLGuiComponent implements DLContextMenu.MenuBuilder {
    ...
    public final ColorProperty clickAreaColor = new ColorProperty(Color.fromInt(0x403399FF), Color.TRANSPARENT);
    public final Property<Padding> contentPadding = new Property<>(Padding.ZERO);
    ...
    public List<ItemEntry> buildContextMenuContents(int x, int y) {
        ...
        entries.add(new DLContextMenu.ItemEntry(TextUtils.translate(DLTranslate.key(Type.GUI, "menu.copy")), Sprite.empty(), true, () -> {
    ...
    protected void setupRichTextComponent(RichTextComponent rtc) {
        if (rtc != null) {
            ...
            rtc.setTextValidator((current, future, input) -> {
                MutableSingle<String> ipt = new MutableSingle<>(input);
    ...
    public LineMarker getLineByLineIndex(int lineIndex) throws IndexOutOfBoundsException {
        ...
        if (getLinesOrdered().isEmpty()) {
            return new LineMarker(0, 0, 0, 1f, (float) Minecraft.getInstance().font.lineHeight, 0, text.get() != null ? text.get().getParagraphAlignment(0) : EAlignment.LEFT);
        }
    ...
    private void initRenderer() {
        ...
        if (!orderedLines.isEmpty()) {
            ...
            currentMarker = new LineMarker(0, 0, 0, 1f, Minecraft.getInstance().font.lineHeight, 0, text.get() != null ? text.get().getParagraphAlignment(0) : EAlignment.LEFT);
            renderOffsetX = 0;
        }
    }
    ...
    public boolean renderSegment(de.mrjulsen.mcdragonlib.client.util.Graphics graphics, TextSegment segment, int skip) {
    ...
    private boolean renderSegmentInternal(de.mrjulsen.mcdragonlib.client.util.Graphics graphics, TextSegment segment, boolean renderShadow, float segmentStartXLineUnscaledVirtual, int skip) {
    ...
    protected void renderInteractionElementOverlay(Graphics graphics, int mouseX, int mouseY, InteractiveElement element) {
        ...
        GuiUtils.renderTooltip(screen, GuiAreaDefinition.of(screen), lines, screen.width, graphics, (int)mouseX, (int)mouseY);
    }
}
