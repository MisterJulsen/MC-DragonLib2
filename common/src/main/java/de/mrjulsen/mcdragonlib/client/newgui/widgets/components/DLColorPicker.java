...
import de.mrjulsen.mcdragonlib.util.Color.ColorChannel;
...
public class DLColorPicker extends DLGuiComponent {
    ...
    public record ColorChangedEvent(Color color) implements IEvent {}
    ...
    private static enum HSVSlot {
        ...
        public Color colorOf(DLColorPicker instance) {
            return colorOf(instance, this, get(instance));
        }

        public static Color colorOf(DLColorPicker instance, HSVSlot channel, float f) {
            return switch (channel) {
                case H -> Color.fromHsv(f * 360, 1f, 1f);
                case S -> Color.fromHsv(instance.selectedHue * 360, f, 1f);
                case V -> Color.fromHsv(instance.selectedHue * 360, 0f, f);
            };
        }
        
        public static Color colorOf(Pair<HSVSlot, Float> a, Pair<HSVSlot, Float> b, Pair<HSVSlot, Float> c) {
            float h = 360;
            float s = 1;
            float v = 1;
    ...
    public final ColorProperty color = new ColorProperty(Color.WHITE, Color.WHITE)
    ...
    public DLColorPicker(int x, int y, int w, int h) {
        ...
        this.quadTexture = new Cache<>(() -> {
            ...
            for (int i = 0; i < (int)quadArea.get().width(); i++) {
                for (int k = 0; k < (int)quadArea.get().height(); k++) {                
                    ...
                    Color color = HSVSlot.colorOf(Pair.of(hsvSlots.get().quadX(), vX), Pair.of(hsvSlots.get().quadY(), vY), Pair.of(hsvSlots.get().slider(), hsvSlots.get().slider().get(this)));
        ...
        this.colorSliderTexture = new Cache<>(() -> {
            ...
            if (sliderAlign.get().isHorizontal()) {
                for (int k = 0; k < (int)colorSliderArea.get().height(); k++) {
                    ...
                    Color color = HSVSlot.colorOf(this, hsvSlots.get().slider(), val);
                    color = color.swapChannels(ColorChannel.R, ColorChannel.B);
        ...
        addEventListener(DLGuiStandardEvents.DragEvent.class, (s, e) -> {
            ...
            color.set(Color.fromHsv(selectedHue * 360, selectedSaturation, selectedBrightness).withAlpha((int)(selectedAlpha * 255)));
    ...
    public void renderMainLayer(Graphics graphics, double mouseX, double mouseY, Rectangle renderBounds) {
        ...
        if (showAlphaSlider.get()) {            
            ...
            if (sliderAlign.get().isHorizontal()) {
                GuiUtils.fillGradient(graphics, alphaSliderArea.get(), Color.TRANSPARENT, color.get().withAlpha(255), EAlign.TOP);
            } else {
                GuiUtils.fillGradient(graphics, alphaSliderArea.get(), Color.TRANSPARENT, color.get().withAlpha(255), EAlign.LEFT);
            }
        }

        GuiUtils.drawBox(graphics, getQuadCursorX() - 3, getQuadCursorY() - 3, 6, 6, color.get(), Color.pickBasedOnBrightness(color.get(), Color.WHITE, Color.BLACK, 0.5f));
        
        if (sliderAlign.get().isHorizontal()) {
            if (showColorSlider.get()) GuiUtils.drawBox(graphics, (int)colorSliderArea.get().left() - 3, (int)getColorSliderCursor() - 3, (int)colorSliderArea.get().width() + 6, 6, hsvSlots.get().slider().colorOf(this), Color.BLACK);
            if (showAlphaSlider.get()) GuiUtils.drawBox(graphics, (int)alphaSliderArea.get().left() - 3, (int)getAlphaSliderCursor() - 3, (int)alphaSliderArea.get().width() + 6, 6, color.get().withAlpha((int)(selectedAlpha * 255)), Color.BLACK);
        } else {
            if (showColorSlider.get()) GuiUtils.drawBox(graphics, (int)getColorSliderCursor() - 3, (int)colorSliderArea.get().top() - 3, 6, (int)colorSliderArea.get().height() + 6, hsvSlots.get().slider().colorOf(this), Color.BLACK);
            if (showAlphaSlider.get()) GuiUtils.drawBox(graphics, (int)getAlphaSliderCursor() - 3, (int)alphaSliderArea.get().top() - 3, 6, (int)alphaSliderArea.get().height() + 6, color.get().withAlpha((int)(selectedAlpha * 255)), Color.BLACK);
        }
