package de.mrjulsen.mcdragonlib.client.newgui.builtin;
...
import de.mrjulsen.mcdragonlib.client.newgui.widgets.components.DLColorPicker;
...
public class DLColorPickerWindow extends DLWindow {
    ...
    public DLColorPickerWindow(DLWindowManager manager, boolean showAlpha, Color initialColor, Consumer<Color> onAccept) {
        super(manager);
        ...
        picker.color.set(initialColor.isUndefined() ? Color.WHITE : initialColor);
        addComponent(picker);
        ...
        r.addEventListener(DLGuiCommonEvents.ValueChangedEvent.class, (s, e) -> {
            ...
            Color color = picker.color.get();
            colorSpace.selectedItem.get().ifPresent(c -> {
                picker.color.set(switch (c) {
                    case HSV -> Color.fromHsv(r.value.get().floatValue(), color.getSaturation(), color.getBrightness()).withAlpha(color.getAlpha());
                    default -> Color.of(color.getAlpha(), (int)e.value(), color.getGreen(), color.getBlue());
                });
            });
        ...
        g.addEventListener(DLGuiCommonEvents.ValueChangedEvent.class, (s, e) -> {
            ...
            colorSpace.selectedItem.get().ifPresent(c -> {
                picker.color.set(switch (c) {
                    case HSV -> Color.fromHsv(color.getHue(), g.value.get().floatValue() / 100, color.getBrightness()).withAlpha(color.getAlpha());
                    default -> Color.of(color.getAlpha(), color.getRed(), (int)e.value(), color.getBlue());
                });
            });
            skipValueUpdate = false;
            return false;
        ...
        b.addEventListener(DLGuiCommonEvents.ValueChangedEvent.class, (s, e) -> {
            ...
            colorSpace.selectedItem.get().ifPresent(c -> {
                picker.color.set(switch (c) {
                    case HSV -> Color.fromHsv(color.getHue(), color.getSaturation(), b.value.get().floatValue() / 100).withAlpha(color.getAlpha());
                    default -> Color.of(color.getAlpha(), color.getGreen(), color.getGreen(), (int)e.value());
                });
            });
            skipValueUpdate = false;
            return false;
        ...
        hex.addEventListener(DLGuiCommonEvents.ValueChangedEvent.class, (s, e) -> {
            if (skipValueUpdate) return false;
            skipValueUpdate = true;
            picker.color.set(Color.fromInt(hex.value.get().intValue()));
            skipValueUpdate = false;
            return false;
        });
        addComponent(hex);
    ...
    public void renderMainLayer(Graphics graphics, double mouseX, double mouseY, Rectangle renderBounds) {
        ...
        GuiUtils.drawString(graphics, graphics.defaultFont(), WINDOW_PADDING, WINDOW_PADDING, TextUtils.translate("gui." + DragonLib.MODID + ".colorpicker.title"), DragonLib.NATIVE_UI_FONT_COLOR, ETextAlignment.LEFT, false);
    }    
}
