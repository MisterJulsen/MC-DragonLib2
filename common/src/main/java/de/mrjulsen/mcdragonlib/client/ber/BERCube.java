...
public class BERCube {
    ...
    public BERQuad setQuad(ResourceLocation texture, Direction facing, float u0, float v0, float u1, float v1) {
        float w = 0, h = 0, z = 0;
        Quaternionf rot = Axis.YP.rotationDegrees(0);
        switch (facing) {
            case EAST:
                w = depth;
                h = height;
                z = width;
                rot = Axis.YP.rotationDegrees(90);
                break;
            case WEST:
                w = depth;
                h = height;
                z = width;
                rot = Axis.YP.rotationDegrees(270);
                break;
            case UP:
                w = width;
                h = depth;
                z = height;
                rot = Axis.XP.rotationDegrees(90);
                break;
            case DOWN:
                w = width;
                h = depth;
                z = height;
                rot = Axis.XN.rotationDegrees(90);
                break;
            case SOUTH:
                w = width;
                h = height;
                z = depth;
                rot = Axis.YP.rotationDegrees(180);
                break;
            default:
            case NORTH:
                w = width;
